tic
% Define subinterval [-0.019, -0.009] partitioned into 5000 subintervals
n = 5000;                       % number of subdivisions in m
h = intval('0.00001');        % step size in M-direction
delta = 0.000002;             % width of each m-subinterval

W = cell(1,n);                % use cell array to handle variable N(j)

for j = 1:n
    % Define interval endpoints as infsup for rigor
    m_left = -0.009 - j * delta;
    m_right = -0.009 - (j-1) * delta;
...
% Compute max across all cells, max is equal to 0.5074
maxval = max(cellfun(@max, W));
disp(maxval)
toc
%  0.507348296052447, Elapsed time is 629.572520 seconds.