tic
% Define subinterval [-0.0485, -0.0385] partitioned into 1000 subintervals
n = 1000;                       % number of subdivisions in m
h = intval('0.0001');        % step size in M-direction
...
for j = 1:n
    ...
    m_left = -0.0385 - j * delta;
    m_right = -0.0385 - (j-1) * delta;
...
% Compute max across all cells, max is equal to 0.5294
% Elapsed time is 65 seconds
maxval = max(cellfun(@max, W));
disp(maxval)
toc
%0.529309626580661, Elapsed time is 69.313584 seconds.