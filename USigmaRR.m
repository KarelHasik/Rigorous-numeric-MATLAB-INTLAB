function q = USigmaRR(M, m)
% LSigma computes an upper bound for Sigma(m, M) using rigorous INTLAB interval arithmetic
% Inputs: M, m - must be of type 'intval'
% Output: q - upper bound for Sigma(m, M)
...
if ~isa(m, 'intval'); loa=pred(m,2); hia=succ(m,2); m = infsup(loa,hia); end
