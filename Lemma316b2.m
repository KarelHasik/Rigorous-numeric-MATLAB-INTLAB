tic
% Consider subinterval [-0.1385, -0.0485] partitioned into 9000 subintervals
n = 9000;                       % number of subdivisions in m
h = intval('0.00025');        % step size in M-direction
...
for j = 1:n
    ...
    m_left = -0.0485 - j * delta;
    m_right = -0.0485 - (j-1) * delta;
    ...
    N_j = floor(sup(A_j - a2_j) *4*1e3) + 1;
...
% Compute max across all cells, max is equal to 0.5649
% Elapsed time is 1005 seconds
maxval = max(cellfun(@max, W));
disp(maxval)
toc

%  0.564842851920889, Elapsed time is 1078.462273 seconds.