tic
% Define subinterval [-0.0285, -0.0185] partitioned into 500 subintervals
n = 500;                       % number of subdivisions in m
h = intval('0.00002');        % step size in M-direction
delta = 0.00002;             % width of each m-subinterval

W = cell(1,n);                % use cell array to handle variable N(j)

for j = 1:n
    % Define interval endpoints as infsup for rigor
    m_left = -0.0185 - j * delta;
    m_right = -0.0185 - (j-1) * delta;
...
% Compute max across all cells, max is equal to 0.5334
% Elapsed time is 63.649072 seconds
maxval = max(cellfun(@max, W));
disp(maxval)
toc
%0.533391623796746 Elapsed time is 59.119822 seconds.